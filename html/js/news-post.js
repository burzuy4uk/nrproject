document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById("post-date").textContent = new Date(post.date).toLocaleDateString("en-GB",{day:"2-digit",month:"long",year:"numeric"});
    ...
    document.getElementById("related-list").innerHTML = related.map(x=>`
      <li>
        <a href="./news-post.html?slug=${encodeURIComponent(x.slug)}">${escapeHTML(lang==='uk'?(x.title_uk||x.title_en):(x.title_en||x.title_uk))}</a>
